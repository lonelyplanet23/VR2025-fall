...
void draw_imgui(const matf4& view, const matf4& proj);
...
int main(int argc, char **argv) {
    ...
    GLFWwindow *window = glfwCreateWindow(SCR_WIDTH + GUI_WIDTH, SCR_HEIGHT, "Rendering", nullptr, nullptr);
    if (window == nullptr) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        */
		...
		auto proj_mat = camera.get_projection_matrix((float)SCR_WIDTH, (float)SCR_HEIGHT, 0.1f, 100.0f);
        light_shader.set_matf4("projection", proj_mat);
        light_shader.set_matf4("view", view_mat);
...
void draw_imgui(const matf4& view, const matf4& proj) {
    ...
    ImGui::SetNextWindowPos(ImVec2(SCR_WIDTH, 0), ImGuiCond_Always);
    ImGui::SetNextWindowSize(ImVec2(GUI_WIDTH, SCR_HEIGHT), ImGuiCond_Always);
    ImGui::Begin("Settings Panel", nullptr, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse);

    auto display_matrix = [](const char* label, const matf4& matrix) {
    ...
    display_matrix("View Matrix", view);
    display_matrix("Projection Matrix", proj);
